...
async function loadData() {
	try {
		const fpliRes = await fetch('./json/FPLIDimensions.json');
		if (!fpliRes.ok) throw new Error('FPLI data failed to load');
		fpliData = await fpliRes.json();
		...
		const compatRes = await fetch('./json/FPLI_IMP_Compatibility.json');
...
function loadMetric() {
	...
	// Populate FPLI IPN dropdown

	const dropdown1 = document.getElementById('fpliIPNDropdown');
	for (const key in fpliData) {
		if (fpliData.hasOwnProperty(key)) {
	...
	//Disabling Impeller Dropdown until an FPLI is selected
...
//FPLI Dropdown - FPLI Dimensions
document.getElementById('fpliIPNDropdown').addEventListener('change', function () {
	const selectedIPN = this.value;
	const fpliDetails = fpliData[selectedIPN];
	//alert('You selected: ' + fpliDetails["Eye Diameter"]);
	...
	if (buttonImperial.classList.contains('selected')) {
		document.getElementById("fpliID").value = (fpliDetails["ID Thickness"] * 0.0394).toFixed(2);
		document.getElementById("fpliOD").value = (fpliDetails["OD Thickness"] * 0.0394).toFixed(2);
	}
	else {
		document.getElementById("fpliID").value = (fpliDetails["ID Thickness"]).toFixed(2);
		document.getElementById("fpliOD").value = (fpliDetails["OD Thickness"]).toFixed(2);
	}
...
buttonImperial.addEventListener('click', () => {
	...
	if (metricFlag == 1) {
		if (document.getElementById("fpliID").value) {
			document.getElementById("fpliID").value = (document.getElementById("fpliID").value / 25.4).toFixed(2);
			document.getElementById("fpliOD").value = (document.getElementById("fpliOD").value / 25.4).toFixed(2);
		}
...
function validateAnalyse() {
	...
	if (document.getElementById("avgRPM").value.trim() == "" || !isFloat(document.getElementById("avgRPM").value.trim())) { invalidFields += "Average RPM" + "\n"; invalidCounter++; }
	if (document.getElementById("avgHead").value.trim() == "" || !isFloat(document.getElementById("avgHead").value.trim())) { invalidFields += "Average Head" + "\n"; invalidCounter++; }
...
function analyse() {
	...
	if (buttonImperial.classList.contains('selected')) { // If Imperial
		...
		//var idWear = refWearLife * Math.pow((refD50 / averageD50), exponent1) * (refSolidsWeightPercent / solidsWeightPercent) * Math.pow((refEyeTipSpeed / eyeTipSpeed), exponent2) * (refHead / averageHead) * (fpliID / refFPLI_ID);
		var odWear = refWearLife * Math.pow((refD50 / averageD50), exponent1) * (refSolidsWeightPercent / solidsWeightPercent) * Math.pow((refImpTipSpeed / impTipSpeed), exponent2) * (refHead / averageHead) * (fpliOD / refFPLI_OD);
